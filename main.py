...
def helper(dis):
    ...
    pre = [col for col in pre.values[0]] if not pre.empty else ["No precautions available"]
    ...
    wrkout = [w for w in wrkout.values] if not wrkout.empty else ["No workouts available"]
...
@app.route("/")
def index():
    # Pass the list of diseases to the template
    all_diseases = list(diseases_list.values())
    return render_template("index.html", all_diseases=all_diseases)

@app.route('/predict', methods=['GET', 'POST'])
def predict():
    all_diseases = list(diseases_list.values())  # Always pass diseases list
    
    if request.method == 'POST':
        symptoms_input = request.form.get('symptoms', '').strip()
        print("User input symptoms:", symptoms_input)

        if not symptoms_input or symptoms_input == "Symptoms":
            message = "Please enter valid symptoms."
            return render_template('index.html', message=message, all_diseases=all_diseases)
        ...
        if invalid_symptoms:
            message = f"Wrong inputs or data not found: {', '.join(invalid_symptoms)}"
            return render_template('index.html', message=message, all_diseases=all_diseases)
        ...
        return render_template('index.html', predicted_disease=predicted_disease, dis_des=dis_des,
                               ...
                               workout=workout, all_diseases=all_diseases)
